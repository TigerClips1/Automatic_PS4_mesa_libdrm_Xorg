...
if [[ $answer == "Y" ]]; then
  printf "Deleteing, 64-bit-old make sure you install everything right and fix the patch error you see for mesa libdrm"
  rm -rf 64-bit-old
else
  exit 1
fi

printf "Please run bulid_mesa_old_config32.sh on a 32bit debian distro in a vm"
